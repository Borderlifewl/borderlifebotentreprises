...
from config import LOG_TICKET_LSPD_CHANNEL_ID, CATEGORY_LSPD_COMMAND_MAPPING, MAX_LSPD_TICKETS, LSPD_ROLE_COMMAND_ID, OWNER_FDO_ROLE_ID, MANAGER_FDO_ROLE_ID, LEGAL_MANAGER_FDO_ROLE_ID
import os
...
class TicketLSPDCommandView(View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(TicketLSPDCommandSelect())

class TicketLSPDCommandSelect(Select):
    def __init__(self):
        options = [
            discord.SelectOption(label=key, value=key) for key in CATEGORY_LSPD_COMMAND_MAPPING.keys()
        ]
        super().__init__(placeholder="Sélectionnez une option", options=options, min_values=1, max_values=1)

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_modal(TicketLSPDCommandReasonModal(self.values[0]))


class TicketLSPDCommandReasonModal(Modal, title="Raison du ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        category_id = CATEGORY_LSPD_COMMAND_MAPPING[self.category]
        ...
        roles = [
            discord.utils.get(interaction.guild.roles, id=LSPD_ROLE_COMMAND_ID),
        ...
        if ticket_type == "Command Staff":
            embed = discord.Embed(
            title="Contact Command Staff",
            ...
            embed.set_footer(text="Merci de patienter, un membre du Command Staff vous répondra rapidement !", icon_url="https://www.dropbox.com/scl/fi/7noftydvgoqkqsv1d99vj/Logo_lspd.png?rlkey=093qq9nltl5qtdppgj40dyvbu&st=978muzlm&dl=0&raw=1")

        close_view = CloseTicketLSPDCommandView(ticket_channel, interaction.user.id)
        await ticket_channel.send(embed=embed, content=f"<@&{LSPD_ROLE_COMMAND_ID}> {interaction.user.mention}", view=close_view)
        ...
        await interaction.message.edit(view=TicketLSPDCommandView())

class CloseTicketLSPDCommandView(View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_modal(CloseTicketLSPDCommandModal(self.channel, self.user_id))

class CloseTicketLSPDCommandModal(Modal, title="Raison de la fermeture du ticket"):
    def __init__(self, channel, user_id):
        super().__init__()
        self.channel = channel
        self.user_id = user_id
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if interaction.user.id == self.user_id or any(role.id == LSPD_ROLE_COMMAND_ID for role in interaction.user.roles):
...
class TicketsLSPDCommand(commands.Cog):
    def __init__(self, bot):
        self.bot = bot 
    
    @commands.command()
    @commands.has_permissions(administrator=True)
    async def panel_lspd1(self, ctx):
        embed = discord.Embed(
            title="Ticket L.S.P.D. Command Staff - Borderlife",
            description="Pour contacter les Command Staff merci d'ouvrir un ticket dans cette catégorie. Un membre du Command Staff vous contactera rapidement.",
            color=discord.Color.from_rgb(2, 39, 81)
        )
        ...
        await ctx.send(embed=embed, view=TicketLSPDCommandView())

    @commands.command()
    @commands.has_any_role(LSPD_ROLE_COMMAND_ID, OWNER_FDO_ROLE_ID, MANAGER_FDO_ROLE_ID, LEGAL_MANAGER_FDO_ROLE_ID)
    async def ticket_lspd1_add(self, ctx, member: discord.Member):
        if ctx.channel.category and ctx.channel.category.id in CATEGORY_LSPD_COMMAND_MAPPING.values():
    ...
    @commands.command()
    @commands.has_any_role(LSPD_ROLE_COMMAND_ID, OWNER_FDO_ROLE_ID, MANAGER_FDO_ROLE_ID, LEGAL_MANAGER_FDO_ROLE_ID)
    async def ticket_lspd1_rem(self, ctx, member: discord.Member):
        if ctx.channel.category and ctx.channel.category.id in CATEGORY_LSPD_COMMAND_MAPPING.values():
